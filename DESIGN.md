...
|------------|----------|------|
| 自制只自己用 | 普通目录 | 直接编辑最方便 |
| 第三方不定制 | 普通目录 | 直接覆盖更新即可 |
| 第三方要定制 | **subtree** | 保留本地修改 + 合并上游 |
| 自制要分享 | **subtree** | 独立仓库便于分发 |

**核心原则**：只在"需要合并本地修改和上游更新"时用 subtree。

### 脚本

| 脚本 | 位置 | 功能 |
|------|------|------|
| `add-tool.sh` | 配置仓库 scripts/ | 用 subtree 添加第三方工具 |
| `sync-upstream.sh` | 配置仓库 scripts/ | 同步上游更新 |